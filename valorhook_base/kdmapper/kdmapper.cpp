...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, const std::string& driver_path)
{
	...
	do
	{
		...
		std::cout << "[<] Writing fixed image to kernel" << std::endl;
		std::cout << "[?] kernel image base: 0x" << reinterpret_cast<void*>(kernel_image_base) << std::endl;
		std::cout << "[?] local image base: 0x" << reinterpret_cast<void*>(local_image_base) << std::endl;
		std::cout << "[?] image size: " << image_size << std::endl;

		if (!efi_driver::WriteMemory(kernel_image_base, local_image_base, image_size))
		{
		...
		// Erase PE headers
		efi_driver::SetMemory(kernel_image_base, 0, nt_headers->OptionalHeader.SizeOfHeaders);
...
bool kdmapper::ResolveImports(portable_executable::vec_imports imports)
{
	for (const auto& current_import : imports)
	{
		...
		{
			const uint64_t function_address = efi_driver::GetKernelModuleExport(utils::GetKernelModuleAddress(current_import.module_name), current_function_data.name);
