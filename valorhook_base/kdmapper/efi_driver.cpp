...
bool efi_driver::MemCopy(uint64_t destination, uint64_t source, uint64_t size)
{
...
bool efi_driver::SetMemory(uint64_t address, uint32_t value, uint64_t size)
{
	for (int i = 0; i < size; i++) 
	{
		MemCopy(address + i, (uintptr_t)&value, sizeof(uint32_t));
	}

	return true;
}

bool efi_driver::ReadMemory(uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(reinterpret_cast<uint64_t>(buffer), address, size);
}

bool efi_driver::WriteMemory(uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(address, reinterpret_cast<uint64_t>(buffer), size);
}

uint64_t efi_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size)
{
	if (!size)
		return 0;
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(utils::GetKernelModuleAddress("ntoskrnl.exe"), "ExAllocatePool");
...
bool efi_driver::FreePool(uint64_t address)
{
	//std::cout << "[!] FreePool is temporarily disabled" << std::endl;
	//return true;
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(utils::GetKernelModuleAddress("ntoskrnl.exe"), "ExFreePool");
...
uint64_t efi_driver::GetKernelModuleExport(uint64_t kernel_module_base, const std::string & function_name)
{
...
bool efi_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(uint64_t * out_kernel_function_ptr, uint64_t * out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(utils::GetKernelModuleAddress("win32kbase.sys"), "NtGdiDdDDIReclaimAllocations2");
...
bool efi_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(uint64_t* out_kernel_function_ptr, uint8_t* out_kernel_original_bytes)
{
	...
	if (!kernel_function_ptr || kernel_original_jmp_bytes[0] == 0)
	{
		...
		if (!ReadMemory(kernel_function_ptr, kernel_original_jmp_bytes, sizeof(kernel_original_jmp_bytes)))
			return false;
	}
